...
from model import PEGCN, log_likelihood_loss
from data import get_cali_housing_data, get_GData_data, get_metla_data, get_rick_data, get_generation_data, get_Tokyomortality_data,loader
...
def train(args):
    ...
    for xuhao in range(args.n_xuhao):
        ...
        if dset == 'rick':
            id, c, x, y = get_rick_data(norm_coords=True, norm_x=True, norm_y=False, norm_min_val=min_val)
        if dset == 'metla':
            id, c, x, y = get_metla_data(norm_coords=True, norm_x=True, norm_y=False, norm_min_val=min_val)
        if dset == 'generation':
            id, c, x, y = get_generation_data(norm_coords=False, norm_x=False, norm_y=False, norm_min_val=min_val)
        if dset == 'Tokyomortality':
            id, c, x, y = get_Tokyomortality_data(norm_coords=True, norm_x=False, norm_y=False, norm_min_val=min_val)
        ...
        model = PEGCN(num_features_c=c.shape[1], num_features_x=x.shape[1], k=k, emb_dim=emb_dim, res=res).to(device)
        model = model.float()
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                # var_all.append(variance.cpu().numpy())
        ...
        np.savetxt((path + '/trained/{}/result/test_out.csv'.format(saved_file)), test_out, delimiter=",", header=header1)
        ...
        print(f"Final Test Metrics:xuhao: {xuhao},  MAE: {mae:.4f}, MSE: {mse:.4f}, MAPE: {mape:.4f}, R^2: {r2:.4f}")
        list0 = [mae, mse, mape, r2]
        list1.append(list0)
    print("MEAN:", np.mean(list1, axis=0))
    print("STD:", np.std(list1, axis=0))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', type=str, default='cali',
                        choices=['cali', 'GData', 'metla', 'rick', 'generation','Tokyomortality'])
    parser.add_argument('-loss', '--loss', type=str, default='MAE',
                        choices=['MSE', 'log', 'MAE'])
    parser.add_argument("--lr", type=float, default=0.01, help="learning rate")
    parser.add_argument("--train_size", type=float, default=0.1, help="train ratio")
    parser.add_argument("--epochs", type=int, default=500, help="number of training epochs")
    parser.add_argument("--batch_size", type=int, default=128, help="batch size for training")
    parser.add_argument("--lamba", type=float, default=0.1, help="trade-off parameter for KL divergence and likelihood loss")
